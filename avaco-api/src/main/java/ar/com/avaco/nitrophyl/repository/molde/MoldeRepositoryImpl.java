...
public class MoldeRepositoryImpl extends NJBaseRepository<Long, Molde> implements MoldeRepositoryCustom {
	...
	private String getQueryListMoldes(MoldeFilterDTO filtro) {
		String query = " select *, cast(COUNT(*) OVER() as integer) AS totalRows from ( SELECT string_agg(distinct pt.nombre, ', ') as tiposPieza, string_agg(distinct pi.denominacion, ', ') as piezas, cast(m.id_molde as integer) as id, m.codigo, m.faltantes, m.estado, m.nombre, m.ubicacion, "
				+ " cast(MAX(CASE WHEN md.tipodimension = 'ALTO' THEN md.valordimension ELSE NULL END) as double precision) AS ALTO, "
				+ " cast(MAX(CASE WHEN md.tipodimension = 'ANCHO' THEN md.valordimension ELSE NULL END) as double precision) AS ANCHO, "
				+ " cast(MAX(CASE WHEN md.tipodimension  = 'DIAMETRO' THEN md.valordimension ELSE NULL END) as double precision)AS DIAMETRO, "
				+ " cast(MAX(CASE WHEN md.tipodimension = 'PROFUNDIDAD' THEN md.valordimension ELSE NULL END) as double precision) AS PROFUNDIDAD, "

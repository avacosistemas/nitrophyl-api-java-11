...
public abstract class AbstractCaptchaService implements ReCaptchaService {
    ...
    protected void securityCheck(final String response) {
        ...
        if (reCaptchaAttemptService.isBlocked(getClientIP())) {
            throw new ReCaptchaInvalidException("El cliente super el nmero mximo de intentos fallidos");
        }

        if (!responseSanityCheck(response)) {
            throw new ReCaptchaInvalidException("El response contiene caracteres no vlidos");
        }
    }

...
public class FaqServiceImpl extends NJBaseService<Long, Faq, FaqRepository> implements FaqService {
	...
	public List<Faq> listFavouriteFaqs(String category, String subcategory) {
		return getRepository().findByCategoryEqualsAndSubcategoryEqualsAndFavouriteEqualsOrderByOrderFaqAsc(category,
				subcategory, true);
	}
	...
	public Faq save(Faq entity) {
		List<Faq> faqs = this.repository.findByCategoryEqualsAndSubcategoryEqualsOrderByOrderFaqAsc(
				entity.getCategory(), entity.getSubcategory());
	...
	public void remove(Long id) {
		Faq one = repository.findById(id).orElse(null);
		List<Faq> faqs = this.repository.findByIdNotAndCategoryEqualsAndSubcategoryEqualsOrderByOrderFaqAsc(id,
				one.getCategory(), one.getSubcategory());
		for (int i = 1; i <= faqs.size(); i++) {
			faqs.get(i - 1).setOrderFaq(i);
		}
		repository.saveAll(faqs);

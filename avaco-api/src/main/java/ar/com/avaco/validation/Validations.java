...
//		return isRegexValid(CARGO_POLITICO_PATTERN, cargo);
	...
	public static Boolean isDigitoVerificadorCuitCuilValido(String cuit) {
		...
		// Inicializamos una matriz por la cual se multiplicarn cada uno de los dgitos
		Integer[] serie = { 5, 4, 3, 2, 7, 6, 5, 4, 3, 2 };
		// Creamos una variable auxiliar donde guardaremos los resultados del clculo
		// del nmero validador
		Integer aux = 0;
		// Recorremos las matrices de forma simultanea, sumando los productos de la
		// serie por el nmero en la misma posicin
		for (int i = 0; i < 10; i++) {
			aux += Character.getNumericValue(cuitArray[i]) * serie[i];
		}
		// Hacemos como se especifica: 11 menos el resto de de la divisin de la suma de
		// productos anterior por 11
		aux = 11 - (aux % 11);
		// Si el resultado anterior es 11 el cdigo es 0
		if (aux == 11) {
			aux = 0;
		}
		// Si el resultado anterior es 10 el cdigo no tiene que validar, cosa que de
		// todas formas pasa
		// en la siguiente comparacin.

...
//		}
	...
	public void sendMissingPasswordById(Long id) {
		Optional<Usuario> user = getRepository().findById(id);
		if (user.isPresent()) {
			generateNewPassword(user.get());
		} else {
			throw new UsernameNotFoundException("No se ha podido identificar al usuario");
		}
	}
	...
	private void notifyPasswordNewUser(Usuario user, String tmpass) {
		...
		msg.append("Se le ha asignado una contrasea temporal a su usuario ");
		msg.append(user.getUsername());
		msg.append(".<br>");
		msg.append("La contrasea asignada es: <strong>");
		msg.append(tmpass);
		msg.append(".<br>");
	...
	private void notifyPassword(Usuario user, String tmppas) {
		...
		msg.append(user.getNombreApellido());
		msg.append("<br>Se le ha asignado una contrasea temporal a su usuario ");
		msg.append(user.getUsername());
		msg.append(".<br>");
		msg.append("La contrasea asignada es: <strong>");
		msg.append(tmppas);
	...
	public Usuario getUserWithEmails(Usuario user) {
		Usuario current = getRepository().findById(user.getId()).orElseThrow();
	...
	public void update(Usuario usuario, List<Usuario> impersonables) {
		usuario = getRepository().findById(usuario.getId()).orElseThrow();
